...
| `audio_binding` | `rust_decide_audio_binding` / `decide_audio_binding` | Rust-authoritative in production. Python retains model construction, errors, manual selection, URLs, and a frozen equivalence reference used only by explicit tests. Native failure is fail-closed. |
...
| `media_download_candidate_planning` | `rust_plan_media_download_candidates` / `plan_media_download_candidates` | Implements DASH primary/backup flattening and preferred-audio URL flattening. DASH trims/drops empties and preserves duplicates; preferred audio preserves raw strings and duplicates. Python retains descriptor adaptation, all I/O, and frozen `_py_*` equivalence references; production is fail-closed. |
...
| `quality_policy` | `rust_decide_quality_policy` / `decide_quality_policy` | Implements active-label normalization, all historical DASH quality IDs, choice-index mapping, AVC-cap evaluation, yt-dlp maximum-height intent, and BBDown ordered quality intent. Python retains configuration/argument adaptation and frozen `_py_*` equivalence references; production is fail-closed. |
| `video_stream_ranking` | `rust_select_video_stream` / `select_video_stream` | Implements exact codec/quality/AVC filtering, current two fallback stages, descending quality/bandwidth ranking, stable ties, and selected/ranked original indices. Python retains DASH fetching, stream dictionaries, URLs, and a frozen equivalence reference used only by explicit tests; production is fail-closed. |
| `audio_stream_ranking` | `rust_select_audio_stream` / `select_audio_stream` | Implements only regular audio quality ordering and Hi-Res filtering/fallback. Bandwidth remains intentionally irrelevant and equal quality preserves input order. Python retains DASH fetching, stream dictionaries, and a frozen equivalence reference used only by explicit tests; production is fail-closed. |
