...
class ReleaseVersionConsistencyTest(unittest.TestCase):
    ...
    def test_bundle_and_windows_versions_use_release_representation(self):
        with patch.dict(os.environ, {"BILIKARA_VERSION": "v0.7.0"}, clear=False):
            self.assertEqual(build_bundle._bundle_version(), "v0.7.0")
        self.assertEqual(build_bundle._windows_version_tuple("v0.7.0"), (0, 7, 0, 0))
