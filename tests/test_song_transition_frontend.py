...
"""
    ...
    def test_all_next_surfaces_share_the_exact_rust_generation_transport(self):
        ...
        self.assertIn('handleLocalPlaybackEnded("media-ended", endingSession)', ended)
        self.assertIn('const endingSession = state.hostPlaybackSession', ended)
        self.assertIn('isActiveSplitPlayer(video, audio)', ended.split('await audio.bilikaraPitch.drain()', 1)[1])
