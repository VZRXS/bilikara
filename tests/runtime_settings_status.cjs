...
const context = {
  ...
  elements: { serviceStatusIndicator: "service",
    ffmpegPanelStatusIndicator: "ffmpeg", ffmpegStatusRow: {} },
  t: key => key,
  formatQualityLabel: x => x, formatCacheChipMeta() {}, formatCacheUsage() {},
  localizedBBDownLoginMessage: x => x, formatFFmpegHint: () => "",
...
assert.equal(context.elements.ffmpegStatusRow.hidden,true);
...
assert.equal(indicators.service,"failed", "Enabled FFmpeg errors remain visible");
assert.equal(context.elements.ffmpegStatusRow.hidden,false);
...
console.log("PASS runtime account state, disabled CLI visibility and concise QR copy");
