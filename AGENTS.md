...
- Where a panel has an icon close action, place its single close button inside the panel's upper-right corner. Close controls and adjacent header actions use the ordinary secondary-button background token; do not maintain a separate close-button palette. Clicking must not scale the button; Remote close buttons have no hover effect. Preserve keyboard focus indicators and existing non-dismissible entry gates.
- Dialog titles share an 18px font and 24px line height, reserving at least 32px for the title row. Header actions use 14px text in 32px-high pills; circular close controls share that height and the SVG close mark. Align controls with the title's first-line center using the shared size-derived offset, and retain 20px title/side insets. Long titles may wrap without moving controls down to the whole text block's center.
- Apply background blur only to the outer floating panel, not again to its header, content groups or buttons. Removing nested blur must preserve existing theme colors and fill opacity. Do not add scroll-edge separators to headers or footers. Scrolling must not reveal horizontal rules; ordinary separators between distinct settings groups remain unchanged.
- In the Remote playback sheet, render the collapse action as an icon without a visible button fill, and keep the collapse control centered when space permits and move it left only as needed for adjacent actions. Keep the rating entry openable without pending/submitted status labels so users can revisit their draft; disable submission inside the rating dialog when it is ineligible. Preserve the asynchronous submission guard.
- Remote playback transport, part-selection and setting controls use 44px heights; header actions retain the shared 32px height. Overflowing part selections keep the same pills and add an expand action. Show song titles completely within two lines; longer titles use one scrolling line, with manual scrolling under reduced-motion preferences. Cache progress replaces the uploader in its 20px line and restores it when ready; failures and retry icons must not increase that line's height. Coalesce progress-only paints to once per second without delaying terminal states or playback controls.
- Remote UP, favorites and category entry grids show up to 12 cards per page with responsive columns and unchanged card sizes. Preserve the selected page across unrelated state updates.
