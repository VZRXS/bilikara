(async () => {
  ...
  try {
    ...
    if (result?.schema_version !== 1 || result.stage !== "native-bootstrap" || result.backend !== "rust"
      || !Number.isSafeInteger(result.revision) || result.revision < 0
      || result.host_api_ready !== false || result.persistence_ready !== false || result.playback_ready !== false) {
      throw new Error("原生启动检查返回了不兼容的数据。");
    }
    status.textContent = "Rust AppState 已在应用进程内初始化。完整 Host 功能尚未接入。";
