...
Both dependencies disable default features; no general image renderer is added.
Python `qrcode` is no longer a runtime dependency. Pillow and the frozen playlist
export renderer's built-in QR implementation remain in use.
